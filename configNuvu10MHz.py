...
#Attempting to put the camera into a 20MHz mode from start, without Windows GUI.
...
npxly[:]=134#+2  #Email from nuvu says 134 and 2 overscan lines, but aravis says 128!  When using the Windows GUI to put in 20MHz mode, is 134. And when putting in 10MHz mode, is 128x132.  (x x y)
npxlx=npxly.copy()
npxlx[:]=128
nsuby=npxlx.copy()
...
txt=c.getInitTxt()
txt+=c.setClockRateTxt(10)
txt+=c.makeCommand("seg 1\r")
txt+=c.makeCommand("cdsgain 1\r")
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("rsrt\r")
...
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("rsrt\r")
txt+=c.makeCommand("re -1")#start it streaming
camCommand="R[0xB838]=0x10003;GevHeartbeatTimeout=5000;PixelBusSerialSelect=UART0;TestImageSelector=Off;AcquisitionStop=1;PixelFormat=Mono16;Uart0BaudRate=Baud115200;GevSCPSPacketSize=8164;%s"%txt
